...
export const ConstraintZonesEditor = memo(function ConstraintZonesEditor() {
  ...
  const handleUpdate = (
    ...
    const clamped = (field === 'maxVelocity' || field === 'maxAcceleration')
      ? Math.max(0.01, num)
      : num;
  ...
  return (
    <div className="space-y-3">
      {constraintZones.map((zone, i) => (
        <div
          key={zone.id}
          className="rounded border border-amber-500/10 bg-amber-500/[0.03] p-2.5 space-y-2"
        >
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-mono text-amber-400/60">Zone {i + 1}</span>
            <button
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-zinc-500 w-12 shrink-0">Points</span>
            <select
              value={zone.startWaypointIndex}
              onChange={(e) => handleUpdate(i, 'startWaypointIndex', e.target.value)}
            ...
            >
              {Array.from({ length: maxIndex + 1 }, (_, idx) => idx)
                ...
                .map((idx) => (
                  <option key={idx} value={idx}>Pt {idx}</option>
                ))}
            </select>
            <span className="text-[10px] text-zinc-600">to</span>
            <select
              value={zone.endWaypointIndex}
              onChange={(e) => handleUpdate(i, 'endWaypointIndex', e.target.value)}
              className="flex-1 min-w-0 text-xs px-1.5 py-0.5 rounded"
            >
              {Array.from({ length: maxIndex + 1 }, (_, idx) => idx)
          ...
          {/* Max velocity */}
          <div className="flex items-center gap-2">
            <Gauge size={11} className="text-amber-400/50 shrink-0" />
            <span className="text-[10px] text-zinc-500 w-12 shrink-0">Max Vel</span>
            <input
              type="number"
              step={0.1}
              min={0}
            ...
            />
            <span className="text-[10px] text-zinc-600 w-8 shrink-0">m/s</span>
          </div>

          {/* Max acceleration */}
          <div className="flex items-center gap-2">
            <Zap size={11} className="text-amber-400/50 shrink-0" />
            <span className="text-[10px] text-zinc-500 w-12 shrink-0">Max Acc</span>
            <input
              type="number"
              step={0.1}
              min={0}
              value={zone.maxAcceleration}
              onChange={(e) => handleUpdate(i, 'maxAcceleration', e.target.value)}
              className="w-full text-right"
            />
            <span className="text-[10px] text-zinc-600 w-8 shrink-0">m/s²</span>
          </div>
