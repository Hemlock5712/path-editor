...
export function useKeyboardShortcuts(callbacks: {
  ...
  useHotkeys(
    'delete, backspace',
    () => {
      const { selectedPointIndex } = useSelectionStore.getState();
      const { controlPoints } = usePathStore.getState();
  ...
  useHotkeys('period', () => callbacks.onStepForward(), [
    callbacks.onStepForward,
  ]);
  useHotkeys('comma', () => callbacks.onStepBackward(), [
  ...
  useHotkeys('equal, plus', () =>
    useEditorStore
      .getState()
      .setZoom(Math.min(8, useEditorStore.getState().zoom * 1.2))
  );
  useHotkeys('minus', () =>
    useEditorStore
      .getState()
      .setZoom(Math.max(0.5, useEditorStore.getState().zoom / 1.2))
  );
